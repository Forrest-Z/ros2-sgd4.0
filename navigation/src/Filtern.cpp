...
void filtern(string herkunftDat, string zielDat) {
    ...
    // F�hre die Funktionen aus und �bergebe die ben�tigten Variablen
    ...
    f.open(zielDat, f.app);                                             // �ffne und gehe zum letzten Punkt der Zieldatei
    f << docEnde;                                                       // schreibe das Dokumentende in die Zieldatei
    f.close();                                                          // schlie�e die Zieldatei
}

// Findet die zu dem Tag "SharedGuideDog" geh�renden Nodes und Ways in der Quelldatei und speichert die Zeilennummern in einem Array
void SGDrahmenFinden(string herkunftDat, int rahmen[][2]) {

    ifstream osmdatei(herkunftDat);     // �ffnen der Quelldatei
    ...
    while (getline(osmdatei, zeile)) {                                                           // L�uft solange wie in der Quelldatei Zeilen sind, bewegt sich pro Aufruf von getline() eine Zeile weiter und speichert die aktuelle Zeile in "zeile"
        ...
        if (zeile.find("<node") != string::npos || zeile.find("<way") != string::npos) {
            while (zeile.find("</node>") == string::npos && zeile.find("</way>") == string::npos) {       // L�uft solange bis in der aktuellen Zeile "</node>" oder "</way>" gefunden wird
                ...
                getline(osmdatei, zeile);                                                                   // iteriert weiter durch die Quelldatei und gibt neue Zeilen zur�ck
                zeilennr++;                                                                                 // setzt die Zeilenanzahl um 1 hoch
            }
            if (SGDgefunden == 1) {                                                                       // wenn die Variable "SGDgefunden" = 1 ist dann
                ...
                anzahlSGD++;                                                                                // und z�hle die Anzahl die SGD nach oben    
            }
        }
    }
}
...
void SGDSchreiben(string herkunftDat, string zielDat, int rahmen[][2]) {
    ...
    ifstream osmdatei2(herkunftDat);    // �ffnen der Quelldatei

    while (getline(osmdatei2, zeile)) {                      // L�uft solange wie in der Quelldatei Zeilen sind, bewegt sich pro Aufruf von getline() eine Zeile weiter und speichert die aktuelle Zeile in "zeile"
        zeilennr++;                                             // setzt die Zeilenanzahl um 1 hoch
...
// Finden der zu den Referenzen geh�renden Nodes und Ways
void referenzSuchen(string zielDat, string* refs) {
    ...
    ifstream testErgebnis(zielDat);    // �ffnen der Zieldatei

    while (getline(testErgebnis, zeile)) {                                               // L�uft solange wie in der Zieldatei Zeilen sind, bewegt sich pro Aufruf von getline() eine Zeile weiter und speichert die aktuelle Zeile in "zeile"
        aktuelleZeile = "";                                                                 // Setzt den String "aktuelleZeile" zur�ck
        if (zeile.find(ref) != string::npos) {                                                // wenn in der aktuellen zeile der String "ref" gefunden wird dann
            ...
            for (int i = 0; i <= 100; i++) {                                                      // Solange i kleiner gleich 100 ist f�hre aus
                if (aktuelleZeile[i] == zeilenEnde[0] && aktuelleZeile[i + 1] == zeilenEnde[1]) {      // wenn das zeilenende der aktuellen zeile zu erkennen ist dann
                    ...
                    while (aktuelleZeile[i] != idEnde[0]) {                                               // L�uft solange die aktuelle Zeile an der Stelle i nicht dem Ende der ID entspricht
                        i++;                                                                                // Z�hle i hoch
                        b++;                                                                                // Z�hle b hoch
                    }
                    i = i - b;                                                                          // Setze i um b zur�ck
                    id.resize(b);                                                                       // Erzeugt eine neue Gr��e des Strings "id" mit der Gr��e "b"
            ...
            if (refgefunden == 1) {                                                               // wenn die Variable = 1 ist dann
                ...
                anzahlREF++;                                                                        // Z�hle die Zeilen hoch
            }
        }
    }
}

// �berpr�ft ob die gefundenen Referenzen schon in der Zieldatei sind
void referenzAbgleich(string herkunftDat, int rahmen[][2], int rahmenREF[][2], string* refs, int refSize) {
    ...
    ifstream osmdatei3(herkunftDat);    // �ffnen der Quelldatei

    while (getline(osmdatei3, zeile)) {                                                                                                      // L�uft solange wie in der Quelldatei Zeilen sind, bewegt sich pro Aufruf von getline() eine Zeile weiter und speichert die aktuelle Zeile in "zeile"

        zeilennr++;                                                                                                                             // Z�hlt die Zeilennummer hoch
        ...
        if (zeile.find("<node") != string::npos || zeile.find("<way") != string::npos || zeile.find("<relation") != string::npos) {               // wenn in der aktuellen Zeile "<node", "<way" oder "<relation" gefunden wird dann
            if (zeilennr == rahmen[o][0]) {                                                                                                           // wenn die aktuelle Zeilennummer dem aktuellen Rahmenanfang entspricht dann
                for (int i = 0; i < rahmen[o][1] - rahmen[o][0]; i++) {                                                                                   // solange i kleiner der rahmengr��e ist 
                ...
                while (zeile.find("</node>") == string::npos && zeile.find("</way>") == string::npos && zeile.find("</relation>") == string::npos) {      // Solange das Ende von eines Nodes, eines Ways oder einer Relation nicht gefunden wurde
                    if (zeile.find("<node") != string::npos || zeile.find("<way") != string::npos || zeile.find("<relation") != string::npos) {               // wenn der Anfang einer Node, einem Way oder einer relation gefunden wurde dann
                        ...
                        while (1) {                                                                                                                               // unendlichschleife
                            ...
                            else if (zeile.find(refs[i]) != string::npos) {                                                                                          // sonst wenn in der aktuellen Zeile die Referenz �bereinstimmt 
            ...
            if (refgefunden == 1) {                                                                                                                   // wenn eine Referenz gefunden wurde dann
                ...
                anzahlREF++;                                                                                                                            // Z�hlt die Variable hoch
            }
        }
...
void referenzSchreiben(string herkunftDat, int rahmenREF[][2], string zielDat) {
    ...
    ifstream osmdatei4(herkunftDat);    // �ffnen der Quelldatei

    while (getline(osmdatei4, zeile)) {                      // L�uft solange wie in der Quelldatei Zeilen sind, bewegt sich pro Aufruf von getline() eine Zeile weiter und speichert die aktuelle Zeile in "zeile"
        zeilennr++;                                             // Z�hlt die Variable hoch
